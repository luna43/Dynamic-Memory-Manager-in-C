		...
		while (fscanf(tracefile, "%s", type) != EOF) {
			...
			case 'f':
				...
				if (blocks[index] == NULL) {
					...
					if (debug & verbose) fprintf(stderr, "  Freeing block %u size %I64u\n", index, block_sizes[index]);
					for (int i = 0; i < block_sizes[index]; i++) {
//						if (*((char*)blocks[index]+i) != (char)(index & 0xFF)) {
...
//						}
					...
					elapsed_time += clock()-t;
					if (debug & verbose) fprintf(stderr, "  Freed block %u size %I64u\n", index, block_sizes[index]);
					blocks[index] = NULL;
					block_sizes[index] = 0;
				}
				break;
		...
		for (int i = 0; i < num_ids; i++) {
			if (blocks[index] != NULL) {
				if (debug) fprintf(stderr, "%sblock %d not freed, size=%I64u\n", newline, i, block_sizes[i]);
